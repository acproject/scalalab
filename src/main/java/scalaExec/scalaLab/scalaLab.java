...
public class scalaLab extends JFrame implements WindowListener, ActionListener {
    ...
    private static final JMenuItem toggleInterpreterSwitchBehaviorItem = new JMenuItem("Toggle creation of whole new interpreter on switch.");

    private final JMenuItem resetScalaInterpreterItem = new JMenuItem("Scala Interpreter using Scalalab default Imports");

    private final JMenuItem resetScalaInterpreterNoImportsItem = new JMenuItem("Scala Interpreter not using any ScalaLab related  imports (useful for e.g. avoiding problems with ScalaLab related implicits)");

    private final JMenuItem resetScalaInterpreterEJMLItem = new JMenuItem("Scala Interpreter using Efficient Java Matrix Library of Peter Abeles");

    private final JMenuItem   resetScalaInterpreterCUDAItem = new JMenuItem("Scala Interpreter using CUDA Accelerated matrix type");

    private final JMenuItem resetScalaInterpreterEIGENItem = new JMenuItem("Scala Interpreter using EIGEN Library");

    private final JMenuItem resetScalaInterpreterD2Das1DItem = new JMenuItem("Scala Interpreter using D2Das1D matrix");

    private final JMenuItem resetScalaInterpreterJBLASItem = new JMenuItem("Scala Interpreter using JBLAS Library");
    ...
    private final JMenuItem resetScalaInterpreterMTJItem = new JMenuItem("Scala Interpreter using MTJ  Java Matrix Library ");

    private final JMenuItem resetToolboxesScalaInterpreterItem = new JMenuItem("Scala Interpreter using Scalalab default Imports and installed toolboxes");

    private final JMenuItem clearAllScalaInterpreterItem = new JMenuItem("Scala Interpreter without any default Imports");

    private final JMenuItem resetReplayScalaInterpreterItem = new JMenuItem("Scala Interpreter using Scalalab default Imports, replaying commands");

    private final JMenuItem resetReplayScalaInterpreterEJMLItem = new JMenuItem("Scala Interpreter using Efficient Java Matrix Library of Peter Abeles, replaying commands");

    private final JMenuItem resetReplayScalaInterpreterCommonMathsItem = new JMenuItem("Scala Interpreter using Apache Common Maths Library, replaying commands");

    private final JMenuItem resetReplayScalaInterpreterMTJItem = new JMenuItem("Scala Interpreter using MTJ  Java Matrix Library, replaying commands");

    private final JMenuItem resetReplayToolboxesScalaInterpreterItem = new JMenuItem("Scala Interpreter using Scalalab default Imports and installed toolboxes, replaying commands");

    private final JMenuItem clearReplayAllScalaInterpreterItem = new JMenuItem("Scala Interpreter without any default Imports, replaying last session commands");
    ...
    public void InitJMenuBar(final ActionListener listener) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
               ...
               JMenuItem executeJShell = new JMenuItem("Execute with JShell");
                
                executeJShell.addActionListener((ActionEvent e) -> {

            //       com.github.fracpete.jshell.JShellPanel.createJShellFrame();

                }); 
                  

                
                jShellMenu.add(executeJShell);
                ...
                scalaInterpreterMenu.add(toggleInterpreterSwitchBehaviorItem);
                ...
                toggleInterpreterSwitchBehaviorItem.setFont(GlobalValues.uifont);
            ...
            scalaExec.Interpreter.GlobalValues.interruptcifor=true;
                ...
                toggleInterpreterSwitchBehaviorItem.addActionListener((ActionEvent e) -> {

                	GlobalValues.createNewInterpreterOnToggleLibraries = ! GlobalValues.createNewInterpreterOnToggleLibraries;

                	if (GlobalValues.createNewInterpreterOnToggleLibraries == true) 

                		System.out.println("creating totally new interpreter on library switching");

                	else

                		System.out.println("reusing current interpreter on library switching, after performing the proper import statements");

                		       scalaExec.scalaLab.scalaLab.toggleInterpreterSwitchBehaviorItem.updateUI();
                		       
                		       //setTitle("Create whole new interpreter on switch. Current state is "+scalaExec.Interpreter.GlobalValues.createNewInterpreterOnToggleLibraries);
                ...
                progPatterns objProgPatterns = new progPatterns(mainJMenuBar);
               // objProgPatterns.constructProgPatternsMenuBar();
